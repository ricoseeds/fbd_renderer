...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i <= 400; i++)
	{
		...
		string fpath = mainpath + te + ".obj";
		// std::cout << fpath << "\n";
		mesh[0].loadVerts(fpath, allvs);
	}
	// mesh[0].loadOBJ("models/brain_chopped.obj", true);
	// mesh[1].loadOBJ("models/floor.obj", false);
	...
	// Model positions
	glm::vec3 modelPos[] = {
		glm::vec3(0.0f, 0.0f, 0.0f), // liver
		glm::vec3(0.0f, -6.0f, 0.0f) // floor

	};

	// Model scale
	glm::vec3 modelScale[] = {
		glm::vec3(1.0f, 1.0f, 1.0f),  // liver
	...
	while (!glfwWindowShouldClose(gWindow))
	{
		...
		double ticker = currentTime - tickerTime;
		if (ticker > 0.07)
		{
		...
		// std::cout << "Orbit Camera Pos : " << glm::to_string(orbitCamera.getPosition()) << std::endl;
		...
		for (int i = 0; i < numModels; i++)
		{
			...
			if (i == 0)
			{
				// liverMat = glm::scale(glm::mat4(1.0), modelScale[i]);
				liverMat = glm::scale(glm::mat4(1.0), modelScale[i]) * glm::rotate(glm::mat4(1.0), glm::radians((float)(gModelYaw)), glm::vec3(0.0f, 1.0f, 0.0f)) * glm::rotate(glm::mat4(1.0), glm::radians((float)(gModelPitch)), glm::vec3(1.0f, 0.0f, 0.0f));
				lightingShader.setUniform("model", liverMat);
