...
//-----------------------------------------------------------------------------
void Mesh::loadVerts(std::string &fname, std::vector<std::vector<glm::vec3>> &allvs)
{
	...
	while (std::getline(fin, lineBuffer))
	{
		...
		if (cmd == "v")
		{
			...
			newvts.push_back(vertex);
		}
		if (cmd == "f")
			break;
	}
	// std::cout << newvts[newvts.size() - 1].x << " " << newvts[newvts.size() - 1].y << " " << newvts[newvts.size() - 1].z << "\n";
	allvs.push_back(newvts);
	// std::cout << " newvts " << newvts.size() << "\n";
}

void Mesh::assignVerts(std::vector<glm::vec3> vrts)
{
	tempVertices.clear();
	for (int i = 0; i < vrts.size(); i++)
	{
		tempVertices.push_back(vrts[i]);
	}
}
